...
L.DivHeatmapLayer = L.FeatureGroup.extend({
  ...
  _addBlob: function(latitude,longitude,value,old_marker){
    // Remove previous
    if (typeof old_marker != 'undefined') {
      this.removeLayer(old_marker);
    }

    if ( (!value && value != 0) || (!latitude && latitude != 0) || (!longitude && longitude != 0)) {
  ...
  setData: function(data) {
    // Data object is three values [ {latitude,longitude,value}, {...}, ...]
    this.clearData();
    var self = this;
    data.forEach(function(point){
      point.value = point.value > 1 ? 1 : point.value;
      var marker = self._addBlob(point.latitude,point.longitude,point.value);
  ...
  _animateBlob: function(latitude,longitude,start_value,end_value,marker,fadeIn,fadeOut) {
    var self = this;
    ...
    var seed = setInterval(function() {
      ...
      v < 0 ? v = 0 : v > 1 ? v = 1 : v = v;
      marker = self._addBlob(latitude,longitude,v,marker);
      if (v >= Math.max(start_value,end_value) || v <= Math.min(start_value,end_value)) {
        //console.log(latitude,longitude,v,marker);
  ...
  fadeInData: function (data) {
    ...
    data.forEach(function(point){
      point.value = point.value > 1 ? 1 : point.value;
      self._animateBlob(point.latitude,point.longitude,0,point.value,null,function fadeIn(marker) {
                self._markerset.push(marker);
        self._dataset.push({
          "latitude": point.latitude,
          "longitude": point.longitude,
  ...
  fadeOutData: function() {
    ...
    for (var i =0; i < qty; i++) {
      ...
      self._animateBlob(point.latitude,point.longitude,point.value,0,marker,null,function fadeOut(marker) {
  ...
  //     var latitude = 90 * Math.random();
  //     var longitude = 180 * Math.random();
  //     data.push({
  //       "latitude": latitude,
  //       "longitude": longitude,
  //       "value": value
  ...
  //     var latitude = 90 * Math.random();
  //     var longitude = 180 * Math.random();
  //     this._addBlob(latitude,longitude,value);
  //   }
  // },
